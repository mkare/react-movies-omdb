...
const Button: React.FC<ButtonProps> = ({
  ...
  const classes = classNames(
    "border font-medium rounded-md",
    {
      ...
      "bg-white hover:text-secondary-200 border-transparent":
        variant === "link" && !outline,
    },
    {
      "bg-white/75 text-primary-600 hover:bg-primary-500 hover:text-white hover:bg-primary-400 focus:bg-primary-600 focus:text-white border-primary-600":
        variant === "primary" && outline,
      "bg-white/75 text-secondary-600 hover:bg-secondary-500 hover:text-white hover:bg-secondary-400 focus:bg-secondary-600 focus:text-white border-secondary-600":
        variant === "secondary" && outline,
    ...
    },
    {
      ...
      "px-5 py-2 text-sm": size === "small" && !icon,
      "px-6 py-3 text-base": size === "medium" && !icon,
      "px-8 py-3 text-lg": size === "large" && !icon,
      "px-2 py-3 text-center": icon,
    },
    className
  );
  ...
  return (
    <button className={classes} onClick={onClick} disabled={disabled} type={type}>
      ...
      {children && <div className={icon ? "mx-2" : ""}>{children}</div>}
    </button>
  );
};
